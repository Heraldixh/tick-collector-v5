    ...
    pub fn view<'a>(&self, ticker_info: Option<TickerInfo>) -> Element<'a, Message> {
        ...
        match self.view_mode {
            ViewMode::BasisSelection => {
                ...
                // Footprint is tick-only (no time-based mode)
                let is_footprint = matches!(kind, ModifierKind::Footprint(_, _));
                ...
                if selected_basis.is_some() {
                    ...
                    let tabs_row = {
                        if is_footprint {
                            // Footprint: tick-only, no Timeframe tab
                            row![text("Tick Count").size(13)]
                        } else if allows_tick_basis {
                ...
                // For Footprint, always show tick options (ignore tab selection)
                let effective_tab = if is_footprint {
                    SelectedTab::TickCount {
                        raw_input_buf: NumericInput::default(),
                        parsed_input: None,
                        is_input_valid: true,
