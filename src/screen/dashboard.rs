...
impl Dashboard {
                    ...
                    if to_sync {
                        if let Some(state) = self.get_pane(main_window.id, window, pane) {
                            ...
                            let clusters_cfg = match &state.content {
                                pane::Content::Footprint { chart: Some(c), .. } => {
                                    if let data::chart::KlineChartKind::Footprint { clusters, .. } = c.kind() {
                                        Some(*clusters)
                                    } else {
                                        None
                                    }
                                ...
                                .for_each(|(_, _, state)| {
                                    ...
                                    if should_apply {
                                        ...
                                        if let Some(cluster_kind) = &clusters_cfg
                                            && let pane::Content::Footprint { chart, .. } =
                                                &mut state.content
                                            && let Some(c) = chart
                                        {
    ...
    pub fn toggle_trade_fetch(&mut self, is_enabled: bool, main_window: &Window) {
        ...
        self.iter_all_panes_mut(main_window.id)
            .for_each(|(_, _, state)| {
                if let pane::Content::Footprint { chart: Some(c), .. } = &mut state.content {
        ...
        self.iter_all_panes_mut(main_window)
            .for_each(|(_, _, pane_state)| {
                if pane_state.matches_stream(stream) {
                    if let pane::Content::Footprint { chart: Some(c), .. } = &mut pane_state.content {
