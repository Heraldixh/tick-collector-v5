...
pub enum Message {
    ...
    ErrorOccurred(data::InternalError),
    /// Toggle the enabled state for a ticker (ON/OFF switch)
    ToggleTickerEnabled(Ticker, bool),
}
...
impl TickersTable {
    ...
    pub fn ticker_rows(&self) -> &[TickerRowData] {
        &self.ticker_rows
    }
    ...
    fn ticker_card_container<'a>(
        ...
        if let Some(selected_ticker) = &self.expand_ticker_card {
            ...
            if ticker == selected_ticker && exchange == selected_exchange {
                container(expanded_ticker_card(ticker, display_data, is_fav, is_enabled))
                    .style(style::ticker_card)
                    .into()
            } else {
                ticker_card(ticker, display_data, is_enabled)
            }
        } else {
            ticker_card(ticker, display_data, is_enabled)
        }
...
fn ticker_card<'a>(ticker: &Ticker, display_data: &'a TickerDisplayData, is_enabled: bool) -> Element<'a, Message> {
    ...
    let toggle_el = toggler(is_enabled)
        .on_toggle(move |enabled| Message::ToggleTickerEnabled(ticker_copy, enabled))
        .size(16.0);
...
fn expanded_ticker_card<'a>(
    ...
    column![
        row![
            ...
            .style(move |theme, status| { style::button::transparent(theme, status, false) }),
            Space::new().width(Length::Fill).height(Length::Shrink),
            container(toggle_el).padding([0, 4]),
        ]
        .spacing(2)
        .align_y(Alignment::Center),
        row![
...
where
    ...
    FSelect: 'static + Copy + Fn(RowSelection) -> M,
{
    mini_ticker_card_with_toggle(
        exchange,
        label,
        left_action,
        right_label_and_action,
        chip_label,
        on_select,
        None::<(bool, fn(bool) -> M)>,
    )
...
fn mini_ticker_card_with_toggle<'a, M, FSelect, FToggle>(
    exchange: Exchange,
    label: String,
    left_action: Option<RowSelection>,
    right_label_and_action: Option<(&'static str, Option<RowSelection>)>,
    chip_label: Option<&'static str>,
    on_select: FSelect,
    toggle_state: Option<(bool, FToggle)>,
) -> Element<'a, M>
where
    M: 'a + Clone,
    FSelect: 'static + Copy + Fn(RowSelection) -> M,
    FToggle: 'static + Fn(bool) -> M,
{
    ...
    // Add toggle switch on the right side if provided
    if let Some((is_enabled, on_toggle)) = toggle_state {
        let toggle_el = toggler(is_enabled)
            .on_toggle(on_toggle)
            .size(16.0)
            .spacing(4);
