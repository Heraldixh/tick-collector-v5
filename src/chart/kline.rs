...
impl Chart for KlineChart {
    ...
    fn visible_timerange(&self) -> Option<(u64, u64)> {
        ...
        match &chart.basis {
            ...
            Basis::Tick(_) => {
                // For tick-based charts, use the visible datapoints' timestamps
                if let PlotData::TickBased(tick_aggr) = &self.data_source {
                    if tick_aggr.datapoints.is_empty() {
                        return None;
                    }
                    let first_ts = tick_aggr.datapoints.first().map(|dp| dp.kline.time)?;
                    let last_ts = tick_aggr.datapoints.last().map(|dp| dp.kline.time)?;
